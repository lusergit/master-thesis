...
\begin{problem}[Termination of non-relational
  ...
  Given \(\com\in\imp, \rho \in \bCnr\), decide
  \(\semnr{\com}\rho =^? \top\)
\end{problem}
...
\begin{proof}
  ...
  \item If \(\var[y] = \var[x]\) then  \(\eta\var \subseteq \n\) and
    ...
    \(\max (\eta(\var) \sqcap S) = \max(\eta(\var))=
  ...
  and for all \(\eta' \in \mathbb{A}\) with \(\eta \sqsubseteq \eta'\)
  \begin{equation}
    \label{eq:seq1nr}
  ...
  \begin{align*}
    ...
    \text{by \eqref{eq:seq2}, since } \eta_1 = \semnr{\com_1}\eta \sqsubseteq \semnr{\com_1}\eta' \text{ , by monotonicity} \\
  ...
  \begin{align*}    
    ...
    &\max(\semnr{\com \ndet \tru}^{k_2-k_1} ( \semnr{\com \ndet \tru}^{r(k_2-k_1)} \eta_{k_1}) \var[w]) \geq & \mbox{by \eqref{prop2} as }\eta_{k_1} \sqsubseteq \semnr{\com \ndet \tru}^{r(k_2-k_1)} \eta_{k_1}\\
  ...
  \noindent
  However,
  This would contradict the hypothesis \(\semnr{\fix{\com}} \eta \var[y] \neq \infty\). In fact the inequality \eqref{nopp} would imply
  \begin{align*}
...
\(\bCnr_{\com,\rho} \veq (\Var_{\com} \to \Int_{\com,\rho}) \cup
\{ \top, \bot \}\) where
\[
  \Int_{\com,\rho} \veq \{ \interval{a}{b} \mid a, b \in \nat\ \land\
  a \leq b \leq \max\{\max(\rho), \left(\left| \Var_{\com}\right| + 1
  \right) \bound{\com}\}\}.
\]
...
\begin{lemma}
  ...
  \(\eta : \Var \to \Int\), the abstract semantics
