...
of their behaviour. The general idea is that we can approximate the
...
which would make the analysis computaitonally infeasible.  This
...
Abstract interpretation hevely relies on order theory, which we
...
ones. The minimal structure that we will require bothe in the abstract
...
While monotonic conretizations are sufficent to reason about
soundness, more structure is useful to desing a sound and
\emph{accurate} analyser. The standard abstract interpretation
...
soundness by construction of the analyser:

\begin{theorem}[Galois connection properties]\label{th:galoisprop}
  Given a glaois connection \(\tuple{\abstr, C, A, \concr}\) we have:
  \begin{enumerate}
...
connections: \emph{soundness} and \emph{optimality}. Aditionally, from
...
\begin{definition}[Galois Insertion]
  ...
  \item \(\concr\) is injetive: \(\forall a, a'\in A\)
...
\item the first point allows to construct increasing sequences by
  iterating abstract operations that are not necessarly monotonic, and
...
definition and ensures convergence of strinctly increasing chains.
