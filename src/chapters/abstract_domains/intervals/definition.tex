...
\begin{definition}[Integer intervals]
  ...
  \[ \Int \defin \{ \interval{a}{b} \mid a \in \z \cup \{-\infty\}
    ...
    \{\abstract{\bot}\} \] set of integer intervals. We write \(\top\)
  instead of \(\interval{-\infty}{+\infty}\)
\end{definition}
...
\begin{definition}[Abstract integer domain]
  ...
  abstract domain \(\inte\) for program analysis is the variable-wise
  lifting of \(\Int\): \[ \inte \defin (\Var \to \Int_*) \cup \{
    \abstract{\bot} \} \]
\end{definition}
...
\begin{definition}[Interval concretization]
  ...
  \(\inte\) \(\concr[\Int] : \inte \to \poset{\env}\) as
  \begin{align*}
...
\begin{observation}
  If we consider the ordering \(\sqsubseteq\) on \(\inte\)
  s.t.
  \[\forall \eta, \theta \in \inte \quad \eta \sqsubseteq \theta \iff
    \concr[\Int](\eta) \subseteq \concr[\Int](\theta)\] then
  \(\tuple{\inte, \sqsubseteq}\) is a complete lattice.
\end{observation}

\begin{definition}[Interval abstraction]
  We define the \emph{abstraction map} of some numerical set
  \(X \subseteq \n\) into the abstract domain \(\inte\):
  \(\abstr[\Int] : \poset{\n} \to \inte\) as
...
Connection \[\tuple{\concr[\Int], \dom, \inte, \abstr[\Int]}\] between
the concrete domain \(\dom\) and the abstract domain \(\inte\),
resulting

\begin{definition}[Abstract operations]
  We define sound abstract lub and glb operations in the \(\inte\)
  domain:
  \begin{align*}
...
\begin{definition}[Interval semantics]\label{de:intervalsem}
  ...
  \(\semi{\cdot}: \imp \to \inte \to \inte\). For all
  \(\eta: \Var \to \Int_*\),
  \begin{align*}
    \semi{\var \in S}\eta 
    & \veq  
