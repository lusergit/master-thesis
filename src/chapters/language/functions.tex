...
\begin{notation}[Existstential termination]
  We write \[\cc{C}(a_1, \dots, a_k) \ehalts \iff \exists \stt{C,
    \envi{x_1 \mapsto a_1, \dots, x_k \mapsto a_k}} \to^* \rho_t \quad
  \text{for some } \rho_t\in \env\] meaning that in the transition
  system at least one path leads to a final environment
  \(\rho_t\). Dually a programm \(\cc{C}(a_1, \dots, a_k) \nehalts\)
