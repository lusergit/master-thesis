...
\begin{proof}
  We can prove this by induction on the command \(\com\).
  ...
  that \(\bot \in \bA_{\com,\rho}\). Therefore we consider the case
  where
  \(\semi{\var\in S}\rho = \rho[\var \mapsto \rho\var \sqcap
  \Int(S)]\). Here we have again 2 cases
  \begin{itemize}
  \item in case \(\max(S) \in \n\) then
    \begin{equation*}
      \max(\rho\var \sqcap \Int(S)) \leq \max(\rho \var) \leq \max(\rho) + \bound{\var\in S}
    \end{equation*}
    and therefore
    \begin{equation*}
  ...
  \item in case \(\max(S)\not\in \n\) then
    \begin{equation*}
      \max(\rho\var \sqcap \Int(S)) = \max(\rho\var)
    \end{equation*}
