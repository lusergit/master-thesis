...
\begin{proof}
  ...
  lattice with finite chains which contains all the results of the
  interval analysis.  We prove this by carrying a case analysis on
  each variable in \(\Var_{\com}\).
  \medskip
  ...
  \item \(\max(\semi{\com}\rho\var[y]) = \infty\), then the result of
    the analisys \(\semi{\com}\rho\var[y] = \interval{a}{\infty}\)
