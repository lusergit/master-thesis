...
\[\max(\rho) \veq \max \{ \max(\rho(\var)) \mid \var \in \Var_{\com}
  \}.\]
%
Then, when computing \(\semi{\fix{\com}} \rho\) on such \(\rho\)
...
\begin{lemma}
  ...
  \(\semi{\fix{\com}} \eta = \lfp(\lambda \mu . (\eta \sqcup
  \semi{\com}\mu))\)
  % 
...
\begin{proof}
  ...
  % because of Lemma~\ref{le:inc},
  % \(\forall \eta \in \bA, \var[y] \in \Var\) if
  % \(\max(\semi{\com})\eta\var[y]\) and
  % \(\max(\semi{\com}\eta\var[y]) > \bound{\com}\) then
  % \(\exists\var[z] \in \var, h \in \n \mid |h| \leq \bound{\com}\) and
  % both the following statements hold:
  % \begin{enumerate}[label=(\roman*).]
  % \item \(\max(\semi{\com}\eta\var[y]) = \max{\eta\var[z]} + h\)
  % \item for all \(\eta' \in \mathbb{A}\), if
  %   \(\eta' \sqsupseteq \eta\) then
  %   \(\max(\semi{\com}\eta' \var[y]) \geq \max(\eta' \var[z]) +
  %   h\).
  % \end{enumerate}
\end{proof}
