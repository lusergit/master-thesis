...
\begin{definition}[\textbf{Program bound}]
  ...
  a natural number, denoted \(\bound{\com}\in \nat\), defined
  ...
  \begin{align*}
    ...
    & \veq \bound{\com_1} + \bound{\com_2}\\
    %
    \bound{\kleene{\com}} = \bound{\fix{\com}}
    & \veq (|\varsof{\com}|+1) \bound{\com} \\ 
  \end{align*}
  where \(\varsof{\com}\) denotes the set of variables occurring in
  \(\com\). Dually, the \emph{lower bound } associated with a command
  \(\com\in \imp\) is again a natural number, denoted
  \(\lbound{\com}\in \nat\), defined inductively as follows:
  \begin{align*}
    % 
    \lbound{\var \in S}  
    & \veq \begin{cases}
    ...
    & \veq \bound{\com_1} + \bound{\com_2}\\
    %
    \lbound{\kleene{\com}} = \bound{\fix{\com}}
    & \veq (|\varsof{\com}|+1) \bound{\com} \\ 
  \end{align*}
\end{definition}
