    ...
    \item[Case] (\(i \Rightarrow i+1\)). In this case we have to prove
      ...
      \begin{align*}
        \semnr{\com\ndet\tru}(\semnr{\fix\com}\eta) & = \semnr{\com}(\semnr{\fix\com}\eta) \cup \semnr{\fix\com}\eta \\
                                                    ...
                                                    & = \semnr{\fix\com}\eta
      \end{align*}
      now we % can notice that since by hypothesis
      ...
      % \end{align*}
      we preliminarly observe that by calling \(\beta = \semnr{\fix\com}\eta\)
      \begin{equation*}
        |\beta\var| \neq\infty \Rightarrow \semnr{\com\ndet\tru}\beta\var \supseteq \bsemnr{k_1}{k_2}{\com\ndet\tru}\beta\var
      \end{equation*}
      \begin{align*}
        \max(\semnr{\com\ndet\tru}\beta\var) & \leq \max(\beta) + \bonud{\com\ndet\tru} = \max(\beta) + \bonud{\com} \\
