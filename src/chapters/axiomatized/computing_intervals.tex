...
\begin{theorem}
  ...
  % \sqcup \rho}
  computed in \(\inte\) is less precise than the one computed in
  \(\binte{k_1}{k_2}\).
\end{theorem}
...
\begin{proof}
  ...
  \begin{inductive}
    ...
    considering \(k_1 \leq \min(\rho) - \lbound{\com}\) and
    \(k_2 \geq \max(\rho) + \bound{\com}\). Notice that either
    ...
    considering \({k_1 \leq \min(\rho) - \lbound{\var := k}}\) and
    \({k_2 \geq \max(\rho) + \bound{\var := k}}\).  We can notice
    ...
    \({k_2 \geq \max(\rho) + \bound{\var:=\var[y]+k}}\). Notice also
    that
    \(\lbound{\var := \var[y]+k} = k = \bound{\var := \var[y] + k}\)
    and since
    ...
    are considering
    \(k_1 \leq \min(\rho) - \lbound{\com_1 \ndet \com_2}\) and
    \(k_2 \geq \max(\rho) + \bound{\com_1 \ndet \com_2}\).  By
    inductive hypothesis it holds that
    \begin{equation*}
      \semi[\inte]{\com_1}\rho \sqsupseteq \semi[\binte{k_1}{k_2}]{\com_1}\rho
    \end{equation*}
    for all \(k_1 \leq \min(\rho) - \lbound{\com_1}\) and
    \(k_2 \geq \max(\rho) + \bound{\com_1}\). Again by inductive
    hypothesis it holds that
    \begin{equation*}
      \semi[\inte]{\com_2}\rho \sqsupseteq \semi[\binte{k_1}{k_2}]{\com_2}\rho
    \end{equation*}
    for all \(k_1 \leq \min(\rho) - \lbound{\com_2}\) and
    \(k_2 \geq \max(\rho) + \bound{\com_2}\). In particular, both hold
    for
    \({k_1} \leq {\min(\rho) - \lbound{\com_1} - \lbound{\com_2}} =
    {\min(\rho) - \lbound{\com_1 \ndet \com_2}}\) and
    \({k_2} \geq {\max(\rho) + \bound{\com_1} + \bound{\com_2}} =
    {\max(\rho) + \bound{\com_1 \ndet \com_2}}\), i.e., our initial
    choice of \(k_1, k_2\). We can conclude by closure over \(\sqcup\)
    \begin{equation*}
      \semi[\inte]{\com_1 \ndet \com_2}\rho = \semi[\inte]{\com_1}\rho \sqcup \semi[\inte]{\com_2}\rho \sqsupseteq \semi[\binte{k_1}{k_2}]{\com_1}\rho \sqcup \semi[\binte{k_1}{k_2}]{\com_2}\rho = \semi[\binte{k_1}{k_2}]{\com_1\ndet\com_2}\rho
    \end{equation*}
    which is our thesis.
    ...
    \({k_1 \leq \min(\rho) - \lbound{\com_1 \seq \com_2}}\) and
    \({k_2 \geq \max(\com) + \bound{\com_1 \seq \com_2}}\). Recall
    that
    \(\semi[\inte]{\com_1\seq\com_2}\rho = {\left(\semi[\inte]{\com_2}
        \circ \semi[\inte]{\com_1}\right)}\rho\). By inductive
    hypothesis it holds that
    \begin{align}
      \semi[\inte]{\com_1}\rho & \sqsupseteq \semi[\binte{k_1}{k_2}]{\com_1}\rho & \forall k_1 \leq \min(\rho) - \lbound{\com_1} \; \land \; k_2 \geq \max(\rho) + \bound{\com_1}\tag{\dag}\label{eq:ind1}\\
      \semi[\inte]{\com_2}\rho' & \sqsupseteq \semi[\binte{k_3}{k_4}]{\com_2}\rho' & \forall k_3 \leq \min(\rho') - \lbound{\com_2} \; \land \; k_4 \geq \max(\rho') + \bound{\com_2}\tag{\ddag}\label{eq:ind2}
    \end{align}
    ...
    \begin{equation*}
      ...
      \left( \semi[\inte]{\com_2} \circ \semi[\inte]{\com_1}\right)\rho \sqsupseteq
    ...
    \item Otherwise we have to prove that
      ...
      this case by Lemma~\ref{le:inc} and Lemma~\ref{co:inc}
      respectively
      \begin{align*}
        \max(\semi[\inte]{\fix\com}\rho\var[y]) & \leq \max(\rho) + \bound{\fix\com} \\
        \min(\semi[\inte]{\fix\com}\rho\var[y]) & \geq \min(\rho) - \lbound{\fix\com}
      \end{align*}
      Now, we can observe that for every iterand \(\rho_i\) it holds
      that
      \begin{align*}
        \rho_{i+1} & = \semi[\inte]{\com\ndet\tru}\rho_i \\
                   & = {\left(\semi[\inte]{\com\ndet\tru}\right)}^i\rho \\
                   & \sqsubseteq \textstyle\bigsqcup_{i\in\n}{\left(\semi[\inte]{\com\ndet\tru}\right)}^i\rho = \semi[\inte]{\fix\com}\rho  \\
                   & \sqsubseteq \textstyle\bigsqcup_{i\in\n}{\left(\semi[\binte{k_1}{k_2}]{\com\ndet\tru}\right)}^i\rho = \semi[\binte{k_1}{k_2}]{\fix\com}\rho
      \end{align*}
      for all \(k_1 \leq \min(\rho) - \lbound{\fix\com}\) and
      \(k_2 \geq \max(\rho) + \bound{\fix\com}\) by
      Lemma~\ref{le:leq}. Hence for the max and min values it holds
      that
      \begin{align*}
        \max(\rho_i\var[y]) \leq \max(\semi[\inte]{\fix\com}\rho) \leq \max(\rho) + \bound{\fix\com} \\
        \min(\rho) - \lbound{\fix\com} \leq \min(\semi[\inte]{\fix\com}\rho) \leq \min(\rho_i\var[y])
      \end{align*}
      Now we can work by induction on \(\com\ndet\tru\) and \(i\) in
      order to prove the following:
      \(\semi[\inte]{\com\ndet\tru}\rho_i =
      \semi[\binte{k_1}{k_2}]{\com\ndet\tru}\rho_i\) for all
      \(k_1 \leq \min(\rho_i) - \lbound{\com}\) and
      \(k_2 \geq \max(\rho_i) + \bound{\com}\).\ \todo[inline]{Da
        finire, non mi è chiaro come usare l'ipotesi induttiva.}
      % \begin{description}
      % \item[Case] (\(i=0\)). In this case we have to prove that
      %   \(\semi[\inte]{\com\ndet\tru}\rho =
      %   \semi[\binte{k_1}{k_2}]{\com\ndet\tru}\rho\)
      %   this works by induction on \(\com\ndet\tru\) 
      % \end{description}
    \end{enumerate}
