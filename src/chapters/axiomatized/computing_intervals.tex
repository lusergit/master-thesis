...
\begin{proof}
  ...
  \begin{inductive}
    ...
    \end{align}
    By latter equation we can prove that for every \(i\in\n\) it holds
    that
    \begin{equation}
    ...
    \item[Case] (\(i \Rightarrow i+1\)). In this case we can first
      ...
      \begin{equation}\label{eq:last}
        \semi[\inte]{\com}{\left(\lfp(\lambda \mu . \rho \sqcup \semi[\inte]{\com}\mu )\right)} = \rho \sqcup \semi[\inte]{\com}{\left(\lfp(\lambda \mu . \rho \sqcup \semi[\inte]{\com}\mu )\right)} = {\lfp(\lambda \mu . \rho \sqcup \semi[\inte]{\com}\mu )}
      \end{equation}
      ...
      \end{equation}
      To start we can notice that for every
      \(\var[y] \in \Var_{\com}\),
      \(+ \infty \neq \max(\beta\var[y]) > \bound{\fix\com}\) implies
      that \(\max(\beta\var[y]) \leq \max(\rho) + \bound{\fix\com}\)
      by Lemma~\ref{le:inc}. Hence by Definition~\ref{def:minmax}
      \(\max(\beta) \leq \max(\rho) + \bound{\fix\com}\), and by
      calling \(n = \varsof{\com}\) we can notice the following:
      \begin{align*}
        \max(\beta) + \bound{\com} & \leq \max(\rho) + \bound{\fix\com} + \bonud{\com} \\
