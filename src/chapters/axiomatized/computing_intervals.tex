...
\begin{definition}\label{def:boundedbycom}
  ...
  \(\binte{k_1}{k_2}\) where
  \begin{align*}
    k_1 & = \min(\rho) - \lbound{\com} \\
    k_2 & = \max(\rho) + \bound{\com}
  \end{align*}
\end{definition}
...
\begin{proof}
  ...
  \begin{inductive}
    ...
    considering
    \({k_1 \leq \min(\rho) - \lbound{\var\in S}} = {\min(\rho) -
      \lbound{\var\in S}}\) and
    \({k_2 \geq \max(\rho) + \bound{\var\in S}} = {\max(\rho) +
    ...
    considering
    \({k_1 \leq \min(\rho) - \lbound{\var := k}} = {\min(\rho) -
      \lbound{\var := k}}\) and
    ...
    \(n,m\) s.t.
    \begin{align*}
      m & \leq \min(\rho) - \lbound{\com_1} - \lbound{\com_2} \\
      n & \geq \max(\rho) + \bound{\com_1} + \bound{\com_2}
    \end{align*}
    Hence
    \begin{equation*}
    ...
    \item Otherwise we have to prove that
      ...
      \item[Case] (\(i\Rightarrow i+1\)). In this case we have to
        ...
        \end{equation}
        for all \(k_1 \leq \min(\rho'') - \lbound{\com}\) and
        \(k_2 \geq \max(\rho'') + \bound{\com}\). Now we can observe
      ...
      \end{equation*}
      holds for every \(k_1 \leq \min(\rho) - (n+2)\lbound{\com}\) and
      \(k_2 \geq \max(\rho) + (n+2)\bound{\com}\), which is our
      thesis.
    \end{enumerate}
  \end{inductive}
\end{proof}
