...
\begin{proof}
  ...
  \begin{inductive}
    ...
    \begin{align*}
      \min(\rho) - k \leq \min\left(\rho[\var \mapsto \rho\var[y] + k]\right \var[w]) \\
      \max\left(\rho[\var \mapsto \rho\var[y] + k]\right \var[w]) \leq \max(\rho) + k
    \end{align*}
    hence
    \begin{equation*}
    ...
    \item If \(\max(\semi[\inte]{\fix\com}\rho\var[y]) = + \infty\)
      ...
      \end{align*}
    \item Otherwise we have to prove that for all \(\var[y]\in\Var\)
      \({\max(\semi[\inte]{\fix\com}\rho\var[y])} \geq
      {\max(\semi[\binte{k_1}{k_2}]{\fix\com}\rho\var[y])}\) and
      \({\min(\semi[\inte]{\fix\com}\rho\var[y])} \leq
      ...
      \item[Case] (\(i\Rightarrow i+1\)). In this case we have to
        ...
        \end{equation}
        holds for all \(k_1 \leq \min(\rho'') - \nlbound{\com}\) and
        \(k_2 \geq \max(\rho'') + \nbound{\com}\). Now we can observe
        that because of~\eqref{eq:fixtolub} and Lemma~\ref{le:inc} it
        holds that
        \begin{align*}
          \min(\rho'') & \geq \min(\rho) - \lbound{\fix\com} = \min(\rho) - (n+1)\lbound{\com} \geq \min(\rho) - \nlbound{\fix\com}\\
          \max(\rho'') & \leq \max(\rho) + \bound{\fix\com} = \max(\rho) + (n+1)\bound{\com} \leq \max(\rho) + \nbound{\fix\com}
        \end{align*}
        hence for all
        \begin{align*}
          k_1 & \leq  \min(\rho) - \nlbound{\fix\com} \leq \min(\rho) - \lbound{\fix\com} - \lbound{\com} = \min(\rho) - (n+2)\lbound{\com} \\
          k_2 & \geq \max(\rho) + \nbound{\fix\com} \geq \max(\rho) + \bound{\fix\com} + \bound{\com} = \max(\rho) + (n+2)\bound{\com}
        \end{align*}
