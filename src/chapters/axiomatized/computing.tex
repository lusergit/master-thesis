...
The last thing to do is to prove the following Lemma, which will give
us an important property that will help us prove that we can restrict
our domains with restricted versions of themselves which do not allow
infinite ascending chains. Notice that in the following proof we will
refer to \(\semi[\bA]{\cdot}\) as \(\semi{\cdot}\) for a better
reading experience.

\medskip

\noindent
For the following lemmas consider an abstract domain \(\bA\) s.t.
\begin{enumerate}[label=(\Roman*)]
\item\label{inc:hp1} if \(\max(\abstr(S)) = \infty\) then
  \(\max(a \sqcap \abstr(S)) = \max(a)\) where \(S \in \poset{\z}\);
...
\begin{proof}
  ...
  \item If \(\var[y] = \var[x]\) then  \(\eta(\var) \in \Int_*\) and
    ...
    \(\max (\eta(\var) \sqcap \abstr(S)) \leq \max(S)= \bound{\var \in
    ...
    for all \(\eta' \sqsupseteq \eta\), i.e.,
    \[
    \max (\eta'(\var) \sqcap \abstr(S)) = \max(\eta'(\var))=
    \max(\eta'(\var))+0
    \]
...
\begin{corollary}
  ...
  \(\min(\semi{\com}\eta\var[y]) < \lbound{\com}\) then there exists
  a variable \(\var[z] \in\Var\) and an integer \(h\in\z\) such that
...
We would like to have an analogous property for programs that diverge
to \(-\infty\). We can however notice that if we consider the negation
of a program \(-(\com)\) where by \({- : \imp \to \imp}\) we mean the
following inductively-defined function:

\begin{align*}
  -(\var\in S) & \defin \var \in \{-x \mid x \in S\} \\
  -(\var := k) & \defin \var := -k \\
  -(\var := \var[y] + k) & \defin \var := \var[y] + (- k) \\
  % -(\var := \var[y] - k) & \defin \var := \var[y] + k \\
  -(\com_1 + \com_2) & \defin (-\com_1) + (-\com_2) \\
  -(\com_1 ; \com_2) & \defin (-\com_1) ; (-\com_2) \\
  -(\com^*) & \defin (-\com)^* \\
  -(\fix{\com}) & \defin \fix{-\com} \\
\end{align*}