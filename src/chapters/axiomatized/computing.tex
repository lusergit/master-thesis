...
\begin{proof}
  The statements follows from the observation that Lemma~\ref{le:inc}
  works also for \(\Int\) with the usual ordering \(\sqsubseteq\) but
  with a different \(\max : \Int \to \z\) function, where
  \begin{equation*}
    \max(\interval{a}{b}) = a
  \end{equation*}
\end{proof}
