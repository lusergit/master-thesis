...
variable-wise lifting \(\bAn\), where \(\concr[\bA]\) and
\(\concr\) are the concretization maps for \(\bA\) and \(\bAn\)
respectively, while \(\abstr[\bA]\) and \(\abstr\) are the abstraction
maps again for \(\bA\) and \(\bAn\) respectively. Also consider
the set of integers \(\z\) with the usual ordering \(\leq\) and its
complement \(>\), building the complete lattice \(\tuple{\z,
  \leq}\). In such context if \(\forall \theta \in \bAn\),
...
\begin{lemma}
  \label{le:inc}
  Let \(\com\in \imp\).
  % Then consider a function \({\max_{\bA} : \bA \to \z}\) s.t. for
  % all \({a\in\bA}\) it holds that \(x \leq \max_\bA(a)\) for all
  % \({x \in \concr[\bA](a)}\).
  For all \(\eta \in \bAn\) and \(\var[y] \in \Var\), if
  ...
  \item  for all \(\eta' \in \bAn\), if \(\eta' \sqsupseteq \eta\)
    % and \(\semi{\com}\eta' \neq \top\)
    then
...
\begin{proof}
  ...
  \item If \(\var[y] \neq \var\), then for all \(\eta' \in \bAn\) such
    that \(\eta \sqsubseteq \eta'\) it holds
    \[\bot \neq \semi{\var \in S}\eta' = \eta'[\var \mapsto
      {\abstr[\bA](\concr[\bA](\eta(\var)) \cap \concr[\Int](S))}]\]
    and thus
    \begin{equation*}
  ...
  \item If \(\var[y] = \var[x]\) then \(\eta(\var) \in \bA\) and
    \begin{equation*}
  ...
  thus \todo[inline]{Forse non ovvio perchè \(\abstr[\bA](\{k\})\) può
    non rappresentare esattamente \(\{k\}\).}
  \begin{equation*}
  ...
  % \item If \(\var = \var[y]\) then for all \(\eta' \in \bA\) such
  ...
  all \(\eta' \in \bAn\), \(\eta \sqsubseteq \eta'\),
  \[
  ...
  for all \(\eta' \in \bAn\) with \(\eta \sqsubseteq \eta'\)
  \begin{equation}
    \label{eq:seq1}
  ...
  Now, for all \(\eta' \in \bAn\) with \(\eta \sqsubseteq \eta'\) we have that:
  \begin{align*}
