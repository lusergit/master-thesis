...
precise interval representation for each variable in our program.
...
\begin{proof}
  ...
  \noindent
  \textbf{Case} (\(\ell \Rightarrow \ell + 1\)).
  %
  ...
  \geq \max\left(\eta'\var[w]\right) + h\).
  \todo[inline]{Finire}
  ...
  \begin{align*}
    \semi{\fix{\com}} \eta \var[y]
    & = \semi{\com \ndet \tru}^{m-k_1+1} \semi{\fix{\com}} \eta \var[w]\\
