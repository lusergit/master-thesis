...
\begin{lemma}
  \label{le:inc}
  Let \(\bA\) be an abstract domain and \(\bA_\Var\) its variable-wise
  lifting with \(\abstr[\bA]\) and \(\abstr\) the abstraction maps for
  a single and variable-wise respectively and \(\concr[\bA]\) and
  \(\concr\) the concretization maps for a single variable and
  variable-wise respecively, \(\com\in \imp\). Then consider a
  function \({\max_{\bA} : \bA \to \z}\) s.t. for all \({a\in\bA}\) it
  holds that \(x \leq \max_\bA(a)\) for all
  \({x \in \concr[\bA](a)}\).  In such context if
  \(\forall \theta \in \bA_\Var\), \(S \in \Int\) the following two
  properties hold
  \begin{enumerate}[label=(\Roman*)]
  \item\label{inc:hp1} for all \(\var\in\Var\) if
    \(\max_{\Int}(S) = \infty\) then
    \(\max_\bA(\theta(\var) \sqcap \abstr[\bA](\concr[\Int](S))) = \max_\bA(\theta\var)\)
  \item\label{inc:hp2} \(\max_\bA(a \sqcup b) = \max_\bA\{\max_\bA(a), \max_\bA(b)\}\)
  \end{enumerate}
  
  \noindent
  Then for all \(\eta \in \mathbb{A}\) and \(\var[y] \in \Var\), if
...
\begin{proof}
  ...
  \item If \(\var[y] \neq \var\), then for all
    \(\eta' \in \mathbb{A}\) such that \(\eta \sqsubseteq \eta'\) it
    holds
  ...
  and for all \(\eta_1' \in \mathbb{A}\) with
