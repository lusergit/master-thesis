...
\begin{lemma}
  \label{le:inc}
  Let \(\bA\) be an abstract domain and
  \(\bA_\Var = (\Var \to \bA) \cup \{\bot\}\) its variable-wise
  lifting with \(\abstr : \poset{\z} \to \bA\) abstraction map and
  \(\concr : \bA \to \poset{\z}\) concretization map s.t.
  \(\forall \theta \in \bA_\Var\), \(S \in \poset{\z}\)
  \begin{enumerate}[label=(\Roman*)]
  \item\label{inc:hp1} for all \(\var\in\Var\) if
    \(\max(\abstr(S)) = \infty\) then
    \(\max(\theta(\var) \sqcap \abstr(S)) = \max(\theta\var)\)
